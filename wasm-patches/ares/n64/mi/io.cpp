...
auto MI::writeWord(u32 address, u32 data_, u32& cycles) -> void {
  ...
  if(address == 0) {
    ...
    if(data.bit( 7)) {
      io.initializeMode = 0;
      io.repeatModeLatch = 0;
    }
    if(data.bit( 8)) {
      //Setting init-mode arms the one-shot repeat latch and captures the
      //current length field. See libdragon src/mi_memset.S for the contract.
      io.initializeMode = 1;
      io.repeatModeLatch = 1;
