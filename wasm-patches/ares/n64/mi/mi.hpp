...
struct MI : Memory::RCP<MI> {
  ...
  struct IO {
    ...
    //Repeat-mode latch: armed when the CPU sets init-mode (bit 8) on MI_MODE.
    //The very next RDRAM store consumes the latch and triggers the repeat fill.
    n1 repeatModeLatch;
    n7 repeatLength;  //raw field value; bytes filled = repeatLength + 1 (1..128).
  } io;
