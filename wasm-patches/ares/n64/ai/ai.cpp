...
auto AI::main() -> void {
  // WASM optimization: no audio output, so skip per-sample float math and
  // stream->frame() calls. We still need DMA bookkeeping (length decrements,
  // interrupts) so games waiting on AI continue to work.
#if defined(__wasm__)
  while(Thread::clock < 0) {
    if(io.dmaCount && io.dmaLength[0] && io.dmaEnable) {
      // DMA bookkeeping without reading samples
